'use client';

import { useEffect, useState } from 'react';

export function SimpleThemeToggle() {
  const [isDark, setIsDark] = useState(false);
  const [mounted, setMounted] = useState(false);

  // Initialize theme on mount
  useEffect(() => {
    setMounted(true);
    
    // Check for saved theme preference
    const savedTheme = localStorage.getItem('theme');
    console.log('Saved theme:', savedTheme);
    
    let shouldBeDark = false;
    
    if (savedTheme) {
      // If user has explicitly set a theme, use it
      shouldBeDark = savedTheme === 'dark';
      console.log('Using saved theme:', savedTheme);
    } else {
      // If no saved theme, default to light mode (override system preference)
      shouldBeDark = false;
      console.log('No saved theme, defaulting to light mode');
      // Save the default choice
      localStorage.setItem('theme', 'light');
    }
    
    setIsDark(shouldBeDark);
    console.log('Setting isDark to:', shouldBeDark);
    
    // Apply theme immediately
    document.documentElement.classList.remove('dark'); // Always remove first
    if (shouldBeDark) {
      document.documentElement.classList.add('dark');
      console.log('Added dark class');
    } else {
      console.log('Removed dark class (light mode)');
    }
    
    console.log('HTML classes after init:', document.documentElement.className);
  }, []);

  const toggle = () => {
    const newDarkMode = !isDark;
    console.log('Toggling from', isDark ? 'dark' : 'light', 'to', newDarkMode ? 'dark' : 'light');
    
    // Update state
    setIsDark(newDarkMode);
    
    // Save preference
    const newTheme = newDarkMode ? 'dark' : 'light';
    localStorage.setItem('theme', newTheme);
    console.log('Saved theme:', newTheme);
    
    // Apply to DOM - always remove first, then add if needed
    document.documentElement.classList.remove('dark');
    if (newDarkMode) {
      document.documentElement.classList.add('dark');
      console.log('Added dark class');
    } else {
      console.log('Removed dark class (light mode)');
    }
    
    console.log('HTML classes after toggle:', document.documentElement.className);
  };

  // Don't render until mounted to avoid hydration mismatch
  if (!mounted) {
    return <div className="w-9 h-9" />;
  }

  return (
    <button
      onClick={toggle}
      className="p-2 rounded-lg bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors"
      aria-label={`Switch to ${isDark ? 'light' : 'dark'} mode`}
      title={`Currently ${isDark ? 'dark' : 'light'} mode - click to switch`}
    >
      {isDark ? (
        // Sun icon - switch to light
        <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
          <path fillRule="evenodd" d="M10 2a1 1 0 011 1v1a1 1 0 11-2 0V3a1 1 0 011-1zm4 8a4 4 0 11-8 0 4 4 0 018 0zm-.464 4.95l.707.707a1 1 0 001.414-1.414l-.707-.707a1 1 0 00-1.414 1.414zm2.12-10.607a1 1 0 010 1.414l-.706.707a1 1 0 11-1.414-1.414l.707-.707a1 1 0 011.414 0zM17 11a1 1 0 100-2h-1a1 1 0 100 2h1zm-7 4a1 1 0 011 1v1a1 1 0 11-2 0v-1a1 1 0 011-1zM5.05 6.464A1 1 0 106.465 5.05l-.708-.707a1 1 0 00-1.414 1.414l.707.707zm1.414 8.486l-.707.707a1 1 0 01-1.414-1.414l.707-.707a1 1 0 011.414 1.414zM4 11a1 1 0 100-2H3a1 1 0 000 2h1z" clipRule="evenodd" />
        </svg>
      ) : (
        // Moon icon - switch to dark
        <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
          <path d="M17.293 13.293A8 8 0 016.707 2.707a8.001 8.001 0 1010.586 10.586z" />
        </svg>
      )}
    </button>
  );
}